...
- 🛰️ **Automated Data Acquisition**: Integration with Sentinel Hub API for fetching up-to-date satellite imagery.
- 📊 **Index Calculation**: On-the-fly generation of NDVI (Vegetation), NDWI (Water), NDBI (Built-up), and Moisture indices.
- 🗺️ **Interactive Mapping**: High-performance visualization using MapLibre GL JS.
- 🔄 **Scheduler**: Background tasks for periodic data updates without manual intervention.
- 🗄️ **Geospatial Database**: Powered by Supabase (PostgreSQL + PostGIS) for robust vector data handling.
