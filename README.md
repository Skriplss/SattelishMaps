# SattelishMaps - Hackathon Project

## 🎯 Цель проекта
Визуализация и статистический анализ спутниковых данных Copernicus Sentinel-2.5 с использованием интерактивной карты.

## 👥 Команда
- **Дима** - Team Lead / Главный разработчик
- **Мирон** - Разработчик
- **Денис** - Разработчик
- **Сава** - Разработчик

## 🛠 Технологический стек
- **Backend**: Python (Flask/FastAPI)
- **Database**: Supabase (PostgreSQL + Auth + Storage)
- **Frontend**: HTML, CSS, JavaScript (Leaflet.js / OpenLayers для карт)
- **API**: Copernicus Open Access Hub API
- **Deployment**: Docker

## 📋 План разработки

### Этап 1: Настройка инфраструктуры (1-2 часа)
- [ ] Создать структуру проекта
- [ ] Настроить Git репозиторий
- [ ] Создать Docker-окружение
- [ ] Настроить базу данных

### Этап 2: Backend разработка (3-4 часа)
- [ ] Интеграция с Copernicus API
  - [ ] Аутентификация
  - [ ] Получение данных Sentinel-2.5
  - [ ] Парсинг метаданных
- [ ] API endpoints для фронтенда
  - [ ] GET /api/satellite-data
  - [ ] GET /api/statistics
  - [ ] POST /api/search
- [ ] Сохранение данных в БД
  - [ ] Схема базы данных
  - [ ] CRUD операции

### Этап 3: Frontend разработка (3-4 часа)
- [ ] Создание интерактивной карты
  - [ ] Базовая карта (Leaflet/OpenLayers)
  - [ ] Отображение спутниковых слоев
  - [ ] Интерактивные элементы управления
- [ ] Панель статистики
  - [ ] Графики и диаграммы
  - [ ] Фильтры по датам/регионам
- [ ] UI/UX дизайн

### Этап 4: Интеграция и тестирование (1-2 часа)
- [ ] Соединение Frontend + Backend
- [ ] Тестирование функционала
- [ ] Оптимизация производительности

### Этап 5: Презентация (1 час)
- [ ] Подготовка демо
- [ ] Документация
- [ ] Презентационные материалы

## 🚀 Быстрый старт

### Установка зависимостей
```bash
# Backend
cd backend
pip install -r requirements.txt

# Frontend
cd frontend
npm install
```

### Запуск проекта
```bash
# С помощью Docker
docker-compose up

# Или локально
# Backend
python backend/app.py

# Frontend
cd frontend && npm start
```

## 📚 Полезные ссылки
- [Copernicus Open Access Hub](https://scihub.copernicus.eu/)
- [Sentinel-2 Documentation](https://sentinel.esa.int/web/sentinel/missions/sentinel-2)
- [Leaflet.js Documentation](https://leafletjs.com/)
- [OpenLayers Documentation](https://openlayers.org/)

## 📝 Заметки
- API ключ Copernicus нужно получить заранее
- Рекомендуется кэшировать спутниковые данные
- Использовать асинхронные запросы для больших данных